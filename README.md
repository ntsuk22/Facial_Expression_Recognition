...
exp1-ში loss ნელა იკლებდა, ამიტომ ვიფიქრე, რომ lr შემცირება უფრო ზუსტ gradient descent-ს მოგვცემდა. თუმცა lr=1e-4 კონვერგენციას კიდევ უფრო შეანელა — 30 epoch-ში მოდელი ნაკლებს სწავლობდა. dropout=0.5-ის გაზრდამ კი სწავლა ამ მოდელისთვის შეუძლებლად გახადა. ექსპერიმენტმა დაადასტურა: **პრობლემა overfitting არ არის, regularization-ის გაძლიერება ვერ გამოასწორებს.**
...
scheduler გამოვრთე, რომ მენახა რამდენს გვაძლევს CosineAnnealingLR. სტაბილური lr=1e-3 training loss-ს უფრო სტაბილურად ამცირებდა, მაგრამ weight_decay=1e-2 ამ ზომის მოდელისთვის ძალიან მკაცრი აღმოჩნდა — weight-ები იმდენად პატარა დარჩა, რომ მოდელი feature-ების სწავლას ვეღარ ახერხებდა. ეს ყველაზე ცუდი შედეგი გახდა.
...
exp1-ში val_loss 50 epoch-ზე ჯერ კიდევ იკლებდა — plateau ჯერ არ მიღწეულა, ამიტომ epoch-ების გაზრდა გამართლებული იყო. CosineAnnealingLR ავარჩიეთ, რადგან ReduceLROnPlateau-მ lr საერთოდ არ შეცვალა. CosineAnnealingLR გარანტირებულად ამცირებს lr-ს — training-ის ბოლო ფაზაში fine-tuning ეფექტი. rotation-ის 15°→10°-ზე შემცირება exp1-ის უარყოფითი gap-ის პასუხი იყო. gap შემცირდა -0.018-დან -0.012-მდე, val acc ოდნავ გაიზარდა. ეს სწორი მიმართულება იყო.
...
ეს ექსპერიმენტი დიაგნოსტიკური მიზნით ჩავატარე: გვინდოდა გვენახა, შეძლებდა თუ არა მოდელი training set-ის სწავლას. augmentation-ის მოხსნამ train accuracy 63%→79%-მდე გაზარდა — **მოდელს capacity აქვს.** მაგრამ val accuracy შემცირდა (63.9%→63.4%), gap-ი კი +0.156-მდე გაიზარდა. ეს კლასიკური overfitting-ია: მოდელი training სურათებს „იმახსოვრებდა" generalization-ის გარეშე. ეს დასკვნა მნიშვნელოვანია — underfitting-ი აღარ გვაქვს, regularization სჭირდება.
...
exp3-ის gap +0.156 პირდაპირ მიუთითებდა regularization-ის საჭიროებაზე. weight_decay 10-ჯერ გავზარდეთ — L2 regularization weight-ებს პატარა ამპლიტუდასთან ახლოს ინახავს. conv_dropout 0.25→0.40 feature map-ების დონეზე regularization-ს ამძაფრებს, რაც exp3-მა გვაჩვენა, რომ overfitting conv layer-ებშიც ხდება. augmentation exp2-ის ოპტიმალურ დონეზე დავაბრუნეთ. regularization-მა overfitting მოხსნა (gap: +0.156 → -0.002), მაგრამ val acc არ გაიზარდა. **~63–64% DeeperCNN-ის ceiling-ია** — ნებისმიერი regularization ამ ფარგლებში ინახება.
...
ResNetCNN-მა DeeperCNN-ის ceiling გაარღვია (64%→66.7%). skip connections-ი vanishing gradient-ს გადაჭრა, SE attention-ი per-channel importance-ს სწავლობს. **late-stage overfitting პერსისტენტული პატერნია** — ყველა სამ ექსპერიმენტში training-ის ბოლოს gap იზრდება lr→0-სთან ერთად.
...
**რა სცადეთ და რატომ არ გამოვიდა:** lr-ის შემცირება კონვერგენციას ანელებდა, dropout-ის გაზრდა სწავლას კიდევ უფრო ართულებდა, weight_decay-ის გაზრდა weight-ებს „ჩახრჩობდა". **underfitting-ის გამოსავალი მხოლოდ capacity-ის გაზრდაა.**
